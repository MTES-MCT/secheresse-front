...
const index = {
  ...
  async searchRestriction(address: Address | null,
    ...
    if (error?.value || data?.value?.length < 1) {
      const {title, text, icon} = this.handleRestrictionError(error.value);
  ...
  handleRestrictionError(error: FetchError): { title: string, text: string, icon: string } {
    switch (error?.statusCode) {
      case 404:
      case undefined:
        return {
          title: `C’est pour bientôt ...`,
          text: `Malheureusement, nous n’avons pas encore synchronisé les données de votre zone géographique.<br/><br/>Afin de recevoir des informations sur les restrictions, vous pouvez télécharger l’arrêté préfectoral lié à votre adresse !`,
          icon: `ri-timer-line`
        };
      case 409:
        return {
